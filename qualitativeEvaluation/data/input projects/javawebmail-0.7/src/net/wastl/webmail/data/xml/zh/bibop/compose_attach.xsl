  ...
  <xsl:variable name="work" select="/USERMODEL/WORK/MESSAGE[position()=1]"/>
	...
	<FORM ACTION="{$base}/compose/attach?session-id={$session-id}" METHOD="POST" ENCTYPE="multipart/form-data">
	  ...
	  <TABLE width="100%" border="0" cellspacing="2" cellpadding="4">
		...
		<IMG SRC="{$imgbase}/images/icona_composer.gif" BORDER="0" align="absmiddle"/>附加檔案... (<A HREF="{$base}/help?session-id={$session-id}&amp;helptopic=compose-attach">求救</A>)
	      </TD>
	    </TR>
	    <TR>
     	      <TD colspan="2" bgcolor="#697791" height="22" class="testoBianco">
 		日期：<xsl:value-of select="/USERMODEL/STATEDATA/VAR[@name='date']/@value"/>
	      </TD>
	    </TR>
	    <TR>
		...
		<SPAN class="testoGrande">檔案描述：</SPAN><BR/>
		<TEXTAREA NAME="DESCRIPTION" ROWS="4" COLS="20" class="testoNero"></TEXTAREA>
	      </TD>
	    </TR>
	    <TR>
	      <TD class="testoNero" bgcolor="#D1D7E7">
		<INPUT TYPE="SUBMIT" NAME="ADD" VALUE="加入新檔案" class="testoNero"/>&#160;
		<INPUT TYPE="SUBMIT" NAME="DELETE" VALUE="刪除選擇的檔案" class="testoNero"/>
	      </TD>
	    </TR>
	    <TR>
	      <TD colspan="2" bgcolor="#A6B1C0" align="center" height="22" class="testoGrande">
		<xsl:value-of select="/USERMODEL/STATEDATA/VAR[@name='current attach size']/@value"/> of <xsl:value-of select="/USERMODEL/STATEDATA/VAR[@name='max attach size']/@value"/> bytes
	      </TD>
	    </TR>
	    <TR>
	      <TD COLSPAN="2" bgcolor="#697791" height="22">
		<TABLE border="0" cellpadding="0" cellspacing="0"><TR><TD>
		<A HREF="{$base}/compose?session-id={$session-id}&amp;continue=1"><IMG SRC="{$imgbase}/images/back.gif" BORDER="0"/></A></TD><TD><A HREF="{$base}/compose?session-id={$session-id}&amp;continue=1"><SPAN class="testoBianco"> 回到信件編輯視窗...</SPAN></A></TD></TR></TABLE>
	      </TD>
	    </TR>
	  </TABLE>
	</FORM>	
