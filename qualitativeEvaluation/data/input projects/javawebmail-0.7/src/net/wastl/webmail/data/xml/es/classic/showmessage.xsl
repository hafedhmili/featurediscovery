...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:template match="/">
    <HTML>
      <HEAD>
        <TITLE>WebMail Casilla de correos para <xsl:value-of select="/USERMODEL/USERDATA/FULL_NAME"/>: Showing message <xsl:value-of select="/USERMODEL/CURRENT[@type='message']/@id"/></TITLE>
  ...
  <xsl:template match="MESSAGE">
    ...
    <TABLE WIDTH="100%" BGCOLOR="#dddddd" CELLSPACING="0" BORDER="0">
      <TR>
        ...
        <TD ALIGN="right">(<A HREF="{$base}/help?session-id={$session-id}&amp;helptopic=messagelist">Ayuda</A>)</TD>
      </TR>
      <TR>
        ...
        <FONT SIZE="-1">
          ...
          <FORM ACTION="{$base}/folder/showmsg?flag=1&amp;session-id={$session-id}&amp;folder-id={/USERMODEL/CURRENT[@type='folder']/@id}" METHOD="POST">
            <TD>
              ...
              <STRONG>mensaje a la carpeta</STRONG>
              <SELECT NAME="TO">
  ...
  <xsl:template match="FROM">
    <xsl:choose>
      <xsl:when test="contains(.,'&lt;')"><EM><xsl:value-of select="substring-before(.,'&lt;')"/></EM>,
        <xsl:value-of select="substring-before(substring-after(.,'&lt;'),'&gt;')"/></xsl:when>
      <xsl:otherwise>
        <xsl:value-of select="."/>
      </xsl:otherwise>
    </xsl:choose>
