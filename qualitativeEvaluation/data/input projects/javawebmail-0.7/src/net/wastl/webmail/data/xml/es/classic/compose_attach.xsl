...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:template match="/">
    <HTML>
      <HEAD>
        <TITLE>WebMail Casilla para <xsl:value-of select="/USERMODEL/USERDATA/FULL_NAME"/>: Showing message <xsl:value-of select="/USERMODEL/CURRENT[@type='message']/@id"/></TITLE>
        ...
        <FORM ACTION="{$base}/compose/attach?session-id={$session-id}" METHOD="POST" ENCTYPE="multipart/form-data">
          <TABLE BGCOLOR="#dddddd" CELLSPACING="0" CELLPADDING="5" BORDER="0">
            <TR>
              ...
              <TD VALIGN="CENTER"><FONT SIZE="+2"><STRONG>Archivos agregados...</STRONG></FONT> (<A HREF="{$base}/help?session-id={$session-id}&amp;helptopic=compose-attach">Ayuda</A>)<BR/><EM>Fecha: <xsl:value-of select="/USERMODEL/STATEDATA/VAR[@name='date']/@value"/></EM></TD>
            </TR>
            <TR>
              <TD>
              ...
              <TD>
                ...
                <BR/>
                <TEXTAREA NAME="DESCRIPTION" ROWS="4" COLS="79"/>
                <BR/>
