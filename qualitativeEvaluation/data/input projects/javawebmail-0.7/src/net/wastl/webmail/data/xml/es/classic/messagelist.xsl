...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:template match="/">
    <HTML>
      <HEAD>
        <TITLE>WebMail Casilla de correo para <xsl:value-of select="/USERMODEL/USERDATA/FULL_NAME"/>: Message List (Folder <xsl:value-of select="/USERMODEL/CURRENT[@type='folder']/@id"/></TITLE>
  ...
  <xsl:template match="FOLDER"><H3>
      ...
      in folder <xsl:value-of select="@name"/> (<A HREF="{$base}/help?session-id={$session-id}&amp;helptopic=messagelist">Ayuda</A>).
    </H3><xsl:call-template name="navigation"/><HR/><xsl:apply-templates select="MESSAGELIST"/><STRONG>Banderas del mensaje:</STRONG><IMG SRC="{$imgbase}/images/icon-attachment.gif" BORDER="0"/> message with attachment 
      ...
      <TABLE WIDTH="100%" BGCOLOR="#dddddd" CELLSPACING="0" BORDER="0">
        <TR>
          <FONT SIZE="-1">
            <TD>
              <SELECT NAME="MARK">
                <OPTION VALUE="MARK">colocar*</OPTION>
                <OPTION VALUE="UNMARK">borrar</OPTION>
              </SELECT>
              <STRONG>bandera del mensaje</STRONG>
