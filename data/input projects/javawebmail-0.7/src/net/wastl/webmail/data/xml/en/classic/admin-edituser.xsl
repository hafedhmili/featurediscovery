	  ...
	  <TABLE WIDTH="100%" CELLSPACING="0" CELLPADDING="0">
	    ...
	    <TR>
	      <TD COLSPAN="4" BGCOLOR="lightblue" ALIGN="CENTER">
           <EM><FONT SIZE="+1">Password settings</FONT></EM>
          </TD>
	    </TR>
        <xsl:variable name="cptmpl" select="/GENERICMODEL/STATEDATA/VAR[@name='pass change tmpl']/@value"/>
        <TR>
          <TD WIDTH="100%" COLSPAN="4" ALIGN="CENTER">
            <xsl:call-template name="changepass_header">
              <xsl:with-param name="cptmpl" select="$cptmpl"/>
              <xsl:with-param name="sd" select="/GENERICMODEL/STATEDATA/VAR"/>
        ...
        <TR>
          ...
          <TD WIDTH="40%">
            <xsl:call-template name="changepass_input">
              <xsl:with-param name="cptmpl" select="$cptmpl"/>
              <xsl:with-param name="sd" select="/GENERICMODEL/STATEDATA/VAR"/>
              <xsl:with-param name="tag">user password</xsl:with-param>
            </xsl:call-template>
          </TD>
        </TR>
	    <TR>
